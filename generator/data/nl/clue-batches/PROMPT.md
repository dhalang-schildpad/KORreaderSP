<!--
English note: this file is the Dutch-language prompt used to have an LLM
write clues for Dutch answer words. The prompt text and workflow below stay
in Dutch on purpose, since the output (the clues) must be Dutch. See
CONTRIBUTING.md at the repo root for the contributor workflow in English.
-->
...
3. Sla het antwoord op als `generator/data/nl/clue-batches/batch-NNN.tsv`
   (alleen de regels met woorden, zonder code-hekjes of uitleg).
4. Controleer: `python3 generator/clues.py check generator/data/nl/clue-batches/batch-NNN.tsv`
   Plak eventuele foutmeldingen terug in het gesprek en vraag om alleen die regels te herstellen.
5. Als alles OK is: `python3 generator/clues.py merge`, daarna puzzels genereren.
